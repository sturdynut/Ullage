...
/// Minimal SHA-256. CryptoKit is macOS-only and pulling in swift-crypto for one
/// content hash would be the package's only dependency.
public enum SHA256 {
    public static func hexDigest(_ string: String) -> String { hexDigest(Data(string.utf8)) }

    public static func hexDigest(_ message: Data) -> String {
        var h: [UInt32] = [
        ...
        padded.withUnsafeBytes { raw in
            for chunkStart in stride(from: 0, to: raw.count, by: 64) {
                for i in 0..<16 {
                    let o = chunkStart + i * 4
                    w[i] = (UInt32(raw[o]) << 24) | (UInt32(raw[o + 1]) << 16)
                        | (UInt32(raw[o + 2]) << 8) | UInt32(raw[o + 3])
                }
                for i in 16..<64 {
